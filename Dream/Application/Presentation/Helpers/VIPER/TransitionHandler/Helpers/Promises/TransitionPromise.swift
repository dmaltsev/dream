...
class TransitionPromise<T> {
    ...
    func to(preferredTransitionStyle style: TransitionStyle) -> TransitionPromise<T> {
        ...
        postLinkAction { [weak self] in
            ...
            case .navigationController(navigationStyle: let navCase):
                ...
                case .replaceLast:
                    let controllers = Array(navController.viewControllers.dropLast()) + [self.destination]
                    navController.setViewControllers(controllers, animated: self.animated)
                }
            case .present:
    ...
    /// Set hidesBottomBarWhenPushed property
    ///
    /// - Parameter hidesBottomBarWhenPushed: Bool
    /// - Returns: current promise
    func hidesBottomBarWhenPushed(_ hidesBottomBarWhenPushed: Bool) -> TransitionPromise<T> {
        destination.hidesBottomBarWhenPushed = hidesBottomBarWhenPushed
        return self
    }
}
